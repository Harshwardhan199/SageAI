...
const Home = ({ initialShowSettings = false }) => {
  ...
  useEffect(() => {
    if (urlChatId && urlChatId !== currentChat) {
      ...
      chatService
        .getChatMessages(urlChatId)
        .then((msgs) => setMessages(msgs))
        .catch((err) => {
          console.error(
            "Error loading chat from URL:",
  ...
  const promptDeleteProject = (projectId) => {
    ...
    setConfirmModal({
      ...
      message:
        "Are you sure you want to delete this project? All chats and shared settings associated with it will be permanently deleted.",
  ...
  const handleDeleteMessage = async (messageId) => {
    try {
      const targetIndex = messages.findIndex(
        (m) =>
          m._id === messageId || m.id === messageId || m.tempId === messageId,
      );
      if (targetIndex === -1) return;

      const isMongoId =
        typeof messageId === "string" && /^[0-9a-fA-F]{24}$/.test(messageId);
      if (isMongoId && user) {
  ...
  const handlePrompt = async (customText) => {
    const promptValue =
      typeof customText === "string" ? customText : promptText;
            ...
            setMessages((prev) =>
              prev.map((msg) =>
                msg._id === botId
                  ? {
                      ...msg,
                      blocks: [{ type: "chat", content: displayContent }],
                    }
                  : msg,
              ),
            );
          ...
          prev.map((msg) =>
            msg._id === botId
              ? {
                  ...msg,
                  blocks: [
                    { type: "chat", content: `Error: ${error.message}` },
                  ],
                }
              : msg,
          ),
  ...
  useLayoutEffect(() => {
    if (latestBotRef.current && latestUserRef.current && containerRef.current) {
      ...
      const latestBotHeight = latestBotRef.current
        ? latestBotRef.current.getBoundingClientRect().height
        : 0;

      const height =
        clampedVisibleHeight - latestUserHeight - latestBotHeight - 40;
