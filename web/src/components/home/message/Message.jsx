...
import PromptEditor from "../main/PromptEditor";
...
const Message = forwardRef(
  (
    ...
    const targetMsgId = message._id || message.id || message.tempId;
    const isEditing = isUser && Boolean(targetMsgId) && editingMessageId === targetMsgId;

    // Format timestamp
    const formattedTime = new Date(
      message.createdAt || targetMsgId || Date.now()
    ).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
    ...
    return (
      <div
        ...
        className={`w-full flex gap-1 ${
          isUser ? "flex-col justify-end group" : "flex-col justify-start group"
        }`}
      >
        {isEditing ? (
          <div className="w-full my-2">
            <PromptEditor
              ...
              onSend={() => onEditSend && onEditSend(targetMsgId, editedPrompt)}
        ...
        {isUser && !isEditing && (
          <UserMessageActions
            ...
            onDelete={() => onDeleteMessage && targetMsgId && onDeleteMessage(targetMsgId)}
            onEdit={() => onStartEdit && targetMsgId && onStartEdit(targetMsgId, cleanedText)}
          />
        )}
        {!isUser && <BotMessageActions message={message} />}
      </div>
    );
  }
);
